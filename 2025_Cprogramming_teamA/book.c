...
void loadBooks() {
    ...
    // 파일이 있으면 전체 배열을 한 번에 읽어옴
    fread(g_books, sizeof(Book), MAX_BOOKS, fp);  // g_books 배열 크기만큼 읽기
    fclose(fp);                                   // 파일 닫기
...
void manageBooks() {
    ...
    while (1) {                                 // 0 입력할 때까지 무한 반복
        ...
        if (ch == 1) {
            ...
            printf("ID: ");     scanf("%d", &b->id);       // 도서 ID 입력
            printf("제목: ");   scanf("%s", b->title);     // 제목 입력
            printf("저자: ");   scanf("%s", b->author);    // 저자 입력
